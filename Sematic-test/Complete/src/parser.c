...
#include "error.h"
#include "debug.h"
#define MAX_VARIABLES 100 // Số lượng biến tối đa trong một câu lệnh gán đồng thời
...
// }
void compileAssignSt(void) {
  Type* varTypes[MAX_VARIABLES]; // Lưu kiểu của các biến bên trái
  Type* exprTypes[MAX_VARIABLES]; // Lưu kiểu của các biểu thức bên phải
  int varCount = 0, exprCount = 0;

  // Phân tích danh sách biến bên trái
  do {
    varTypes[varCount++] = compileLValue();
  ...
  // Phân tích danh sách biểu thức bên phải
  do {
    exprTypes[exprCount++] = compileExpression();
    if (lookAhead->tokenType == SB_COMMA) {
      eat(SB_COMMA);
    } else {
      break;
  ...
  // Kiểm tra số lượng biến và biểu thức
  if (varCount != exprCount) {
    error(ERR_NUMBER_OF_ASSIGNMENTS, currentToken->lineNo, currentToken->colNo);
  }

  // Kiểm tra tính tương thích kiểu dữ liệu
  for (int i = 0; i < varCount; i++) {
    checkTypeEquality(varTypes[i], exprTypes[i]);
  }
}
...
Type* compileExpression(void) { 
  Type* type;
  if (lookAhead->tokenType == KW_SUM) {
    // Gọi hàm xử lý sum
    return compileSumExpression();
  }
  switch (lookAhead->tokenType) {
  case SB_PLUS:
    eat(SB_PLUS);
...
Type* compileSumExpression(void) {
  ...
  eat(KW_SUM); // Ăn từ khóa sum

  // Phân tích danh sách biểu thức
  do {
    exprType = compileExpression(); // Phân tích từng biểu thức
    checkIntType(exprType);         // Kiểm tra từng biểu thức phải là kiểu INTEGER

    if (lookAhead->tokenType == SB_COMMA) {
      eat(SB_COMMA); // Nếu có dấu phẩy, tiếp tục
    } else {
      break; // Kết thúc danh sách
    }
  } while (1);

  return intType; // Kết quả tổng là kiểu INTEGER
}
